...
def train(train_loader, model, criterion, optimizer, epoch,labels,multi_M):
    ...
    for i, stuff  in enumerate(train_loader):
        if multi_M:
                (image,masked_image,t1_img,t1_masked_img,t2_img,t2_masked_img,sid,target, saffix,sradiomics) = stuff
                img=torch.cat([image, t1_img, t2_img], dim=1)
                masked_img=torch.cat([masked_image, t1_masked_img, t2_masked_img], dim=1)
        ...
        print("input_affix[:,0]: ",input_affix[:,1])
        print("torch.unsqueeze( input_affix[:,0],1): ",torch.unsqueeze( input_affix[:,1],1))
...
def get_weights_for_balanced_classes(dataset,nclasses,multi_M):
    ...
    for i, item in enumerate(dataset):
        if opt.train_form =="masked_constrained_train":
            ...
            if multi_M:
                (img,t1_img,t2_img,_,_,_ ,_,target, saffix, sradiomics) = item
            else:
...
def draw_lr_change():
    ...
    model = ResNet18(num_classes=2,input_channel=1,use_radiomics=opt.use_radiomics,feature_align=True,use_clinical=opt.use_clinical)
    optimizer = optim.Adam(model.parameters(), lr = opt.lr, weight_decay = 0.5)#decay=0
    ...
    x=[]
    still=False
    for i in range(opt.epochs+100):
        scheduler.step(i)
        for param_group in optimizer.param_groups:
                print("\nepoch: %d "%i+"*learning rate {:.2e}*\n" .format(param_group['lr']))
                x.append(param_group['lr'])
    
    plt.plot(x,'b-',label='t_loss')
    plt.savefig("/home/chenxr/lr.png")
    plt.show()
    assert 0==1
...
def custom_lr_scheduler(epoch):
        ...
        if epoch < 20:
            ...
            lr = (lr_min + 0.5*(lr_max-lr_min)*(1.0+math.cos( (epoch-250)/(T_max-warm_up_iter)*math.pi)))/0.1
...
def main(output_path):
    ...
    #total_comman_total_file=[1, 2, 59, 61, 121, 122] #32,33,41,50,53,83,89,103
    ...
    train_func_dict={"None": train
                    ...
                    ,"single_batchavg_train":single_batchav_train
                    }
    ...
    valid_func_dict={"None": validate
                ...
                ,"single_batchavg_train":single_batchavg_validate
                }
    ...
    for fold, (train_idx,val_idx) in enumerate(splits.split(np.arange(len(total_comman_total_file)),y_box)):
        ...
        if opt.model == "ResNet18":
            ...
            #orig_path ="/home/chenxr/Pineal_region/after_12_08/Results/0704_T1C/Two/new_data/none_sampler_Composed_ResNet18/model_result/"
                ...
                checkpoint = torch.load(best_pretrained_model_path)
                model.load_state_dict(checkpoint['state_dict'])
            ...
            model = ResNet34(num_classes=num_classes,input_channel=input_channel,use_radiomics=opt.use_radiomics,feature_align=feature_align,use_clinical=opt.use_clinical)
        elif opt.model == "ResNet10":
            model = ResNet10(num_classes=num_classes,input_channel=input_channel,use_radiomics=opt.use_radiomics,feature_align=feature_align,use_clinical=opt.use_clinical)
        ...
        for epoch in range(opt.epochs):
            ...
            train_loss, train_losses, train_acc, train_accs,train_met,[train_loss_0,train_loss_1,train_loss_2,train_loss_3]= training(train_loader = train_loader
                                                , model = model
                                                , criterion = criterion
                                                , optimizer = optimizer
                                                , epoch = epoch
