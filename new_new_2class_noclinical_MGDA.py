...
from utils2.self_KL import SelfKL,Self_L0,Self_L1,Self_L2,Self_L3
"""
...
def train(train_loader, model, criterion, optimizer, epoch,labels,multi_M):
...
def All_MGDA_train(train_loader, model, criterion, optimizer, epoch,n_iter=0,writer = None,labels=[0,1],tasks=["CE","SingleBatchAvg","BatchAvg"],loss_fn = {"CE":spare_criterion,"SelfKL":spare_selfKL_criterion,"SingleBatchAvg":singlebatchavg_class_criterion,"BatchAvg":batchavg_class_criterion}):
    orig_tasks=tasks
    losses = AverageMeter()
    maes = AverageMeter()
    Loss_0 = AverageMeter()
...
def All_MGDA_validate(valid_loader, model, criterion,n_iter=0,writer = None,labels =None, multi_M=True ,ps='valid',tasks=["CE","SingleBatchAvg","BatchAvg"],loss_fn = {"CE":spare_criterion,"SelfKL":spare_selfKL_criterion,"BatchAvg":batchavg_class_criterion,"SingleBatchAvg":singlebatchavg_class_criterion}):
...
def Matrix_MGDA_validate(valid_loader, model, criterion,n_iter=0,writer = None,labels =None, multi_M=True ,ps='valid',tasks=['L0','L1','L2','L3'],loss_fn = {'L0':Self_L0(),'L1':Self_L1(),'L2':Self_L2(),'L3':Self_L3()}):
    ...
    with torch.no_grad():
        ...
        for i, (img,auged_img,sid,target, saffix, sradiomics) in enumerate(valid_loader):
            ...
            loss=losses_box[0]+losses_box[1]+losses_box[2]+losses_box[3]
            
            loss_box={"L0":losses_box[0],"L1":losses_box[1],"L2":losses_box[2],"L3":losses_box[3]}
            num_val_batches+=1
            num_val_items+=input_img.size(0)
...
def Matrix_MGDA_train(train_loader, model, criterion, optimizer, epoch,n_iter=0,writer = None,labels=[0,1],tasks=['L0','L1','L2','L3'],loss_fn = {'L0':Self_L0(),'L1':Self_L1(),'L2':Self_L2(),'L3':Self_L3()}):
    orig_tasks=tasks
    losses = AverageMeter()
    maes = AverageMeter()
    Loss_0 = AverageMeter()
    ...
    for i, (img,auged_img,sid,target, saffix, sradiomics) in enumerate(train_loader):
        ...
        for t in tasks:
            ...
            new_target=(F.one_hot(target,num_classes=out_t.size(1))).float()  
            new_target=torch.clamp(new_target,min=0.0001,max=1.0)   
            new_out=F.softmax(out_t,dim=1)
            new_auged_out=F.softmax(auged_out,dim=1)
        ...
        losses_box=[]
        for i, t in enumerate(tasks):
            loss = loss_fn[t](new_out, new_target)
            bloss = loss_fn[t](new_auged_out, new_target)
            loss_t=loss+bloss
            loss_t=loss_t/2.0
            loss_data[t] = loss_t
...
def main(output_path):
    ...
    train_func_dict={"SelfKL":Matrix_MGDA_train,
                     "":All_MGDA_train,
                     "None":All_MGDA_train,
                     "none":All_MGDA_train
                    }
    MGDA_train = train_func_dict[opt.train_form]
    valid_func_dict={"SelfKL":Matrix_MGDA_validate,
                     "":All_MGDA_validate,
                     "None":All_MGDA_validate,
                     "none":All_MGDA_validate
                }
    MGDA_validate = valid_func_dict[opt.train_form]
    ...
    for fold, (train_idx,val_idx) in enumerate(splits.split(np.arange(len(total_comman_total_file)),y_box)):
        ...
        if TEST==True:
            ...
            model.eval()
            train_loss, train_losses, validating_train_acc, train_accs,train_met,[train_loss_0,train_loss_1,train_loss_2,train_loss_3],train_mets= validating(valid_loader = train_loader, model = model, criterion = criterion, labels = labels, multi_M= multi_M)
            ...
            valid_loss, valid_losses, valid_acc, valid_accs, valid_met,[valid_loss_0,valid_loss_1,valid_loss_2,valid_loss_3],valid_mets= validating(valid_loader = valid_loader, model = model, criterion = criterion, labels = labels, multi_M= multi_M)
            ...
            test_loss, test_losses, test_acc, test_accs, test_met,[test_loss_0,test_loss_1,test_loss_2,test_loss_3],test_mets= validating(valid_loader = test_loader, model = model, criterion = criterion, labels = labels, multi_M= multi_M)
        ...
        for epoch in range(opt.epochs):
            ...
            train_loss, train_losses, train_acc, train_accs,train_met,[train_loss_0,train_loss_1,train_loss_2,train_loss_3],n_iter,scale= MGDA_train(train_loader = train_loader
                                                , model = model
                                                , criterion = criterion
                                                , optimizer = optimizer
                                                , epoch = epoch
            ...
            train_loss, train_losses, train_acc, train_accs,train_met =validate(valid_loader = train_loader
                                        ...
                                        , criterion = criterion)
            """
            train_loss, train_losses, validating_train_acc, train_accs,train_met,[train_loss_0,train_loss_1,train_loss_2,train_loss_3],train_mets= MGDA_validate(valid_loader = train_loader, model = model, criterion = criterion, 
            ...
            valid_loss, valid_losses, valid_acc, valid_accs, valid_met,[valid_loss_0,valid_loss_1,valid_loss_2,valid_loss_3],valid_mets= MGDA_validate(valid_loader = valid_loader, model = model, criterion = criterion,
                                                                                                                                                    labels = labels, multi_M= multi_M
                                                                                                                                                    , n_iter = n_iter
                                                                                                                                                    , writer = single_loss_writer
                                                                                                                                                            , ps='validate')
            ...
            test_loss, test_losses, test_acc, test_accs, test_met,[test_loss_0,test_loss_1,test_loss_2,test_loss_3],test_mets= MGDA_validate(valid_loader = test_loader, model = model, criterion = criterion, 
                                                                                                                                          labels = labels, multi_M= multi_M
                                                                                                                                          , n_iter = n_iter
                                                                                                                                        , writer = single_loss_writer
                                                                                                                                            , ps='test')
        ...
        train_loss, train_losses, train_acc, train_accs,train_met,[train_loss_0,train_loss_1,train_loss_2,train_loss_3],train_mets =MGDA_validate(valid_loader = train_loader
                                    , model = model
                                    , criterion = criterion
                                    , labels = labels
                                    , multi_M= multi_M)
        ...
        valid_loss, valid_losses, valid_acc, valid_accs, valid_met,[valid_loss_0,valid_loss_1,valid_loss_2,valid_loss_3],valid_mets= MGDA_validate(valid_loader = valid_loader
                                    , model = model
                                    , criterion = criterion
                                    , labels = labels
                                    , multi_M= multi_M)
        ...
        test_loss, test_losses, test_acc, test_accs, test_met,[test_loss_0,test_loss_1,test_loss_2,test_loss_3] ,test_mets = MGDA_validate(valid_loader = test_loader
                                    , model = model
                                    , criterion = criterion
                                    , labels = labels
                                    , multi_M= multi_M)
