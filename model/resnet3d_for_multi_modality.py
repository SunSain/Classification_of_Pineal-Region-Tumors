...
class ResNet(nn.Module):
    def __init__(
        ...
        self.t2_conv1 = nn.Conv3d(input_channel, self.inplanes, kernel_size=7, stride=2, padding=3, bias=False)
        self.t2_bn1 = norm_layer(self.inplanes)
        self.t2_relu = nn.ReLU(inplace=True)
        self.t2_maxpool = nn.MaxPool3d(kernel_size=3, stride=2, padding=1)
        self.t2_layer1 = self._make_layer(block, 64, layers[0])
        self.t2_layer2 = self._make_layer(block, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0])
        self.t2_layer3 = self._make_layer(block, 256, layers[2], stride=2, dilate=replace_stride_with_dilation[1])
        self.t2_layer4 = self._make_layer(block, 512, layers[3], stride=2, dilate=replace_stride_with_dilation[2])
        self.t2_avgpool = nn.AdaptiveAvgPool3d((1, 1,1))
        
        
        self.t1c_conv1 = nn.Conv3d(input_channel, self.inplanes, kernel_size=7, stride=2, padding=3, bias=False)
        self.t1c_bn1 = norm_layer(self.inplanes)
        self.t1c_relu = nn.ReLU(inplace=True)
        self.t1c_maxpool = nn.MaxPool3d(kernel_size=3, stride=2, padding=1)
        self.t1c_layer1 = self._make_layer(block, 64, layers[0])
        self.t1c_layer2 = self._make_layer(block, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0])
        self.t1c_layer3 = self._make_layer(block, 256, layers[2], stride=2, dilate=replace_stride_with_dilation[1])
        self.t1c_layer4 = self._make_layer(block, 512, layers[3], stride=2, dilate=replace_stride_with_dilation[2])
        self.t1c_avgpool = nn.AdaptiveAvgPool3d((1, 1,1))
        ...
        if use_radiomics:
            self.fc = nn.Linear((512+2+128) * block.expansion, num_classes)
        else:
            self.fc = nn.Linear((512*3+2) * block.expansion, num_classes)
    ...
    def _forward_impl(self, x1: Tensor, x2: Tensor,xc: Tensor,input_affix: Tensor,radiomics:Tensor) -> Tensor:
        ...
        x2 = self.t2_bn1(x2)
        x2 = self.t2_relu(x2)
        x2 = self.t2_maxpool(x2)

        x2 = self.t2_layer1(x2)
        x2 = self.t2_layer2(x2)
        x2 = self.t2_layer3(x2)
        x2 = self.t2_layer4(x2)

        x2 = self.t2_avgpool(x2)
        x2 = torch.flatten(x2, 1)

        xc = self.t1c_conv1(xc)
        xc = self.t1c_bn1(xc)
        xc = self.t1c_relu(xc)
        xc = self.t1c_maxpool(xc)

        xc = self.t1c_layer1(xc)
        xc = self.t1c_layer2(xc)
        xc = self.t1c_layer3(xc)
        xc = self.t1c_layer4(xc)

        xc = self.t1c_avgpool(xc)
        xc = torch.flatten(xc, 1)
        
        x =  torch.cat((x1, input_affix),1)
        x =  torch.cat((x, x2),1) 
        x =  torch.cat((x, xc),1)   
    ...
    def forward(self, x1: Tensor, x2: Tensor,xc: Tensor, input_affix: Tensor,radiomics:Tensor) -> Tensor:
        return self._forward_impl(x1,x2,xc,input_affix,radiomics)
