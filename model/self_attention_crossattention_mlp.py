...
class SelfAttention_CrossAttention_Transformer(nn.Module):
    def __init__(self,dim=514, out_dim=256, new_arch=False,head=1,num_classes=2, depth=1):
        super(SelfAttention_CrossAttention_Transformer, self).__init__()
        self.dim = dim
        self.out_dim = out_dim
        self.head = head
        ...
        self.mlp_head = nn.Sequential(nn.LayerNorm(out_dim*3*2), 
                                      nn.Linear(out_dim*3*2, num_classes))
        inner_dim=out_dim
    ...
    def forward(self,t1,t2,t1c):
        ...
        atten_1,atten_2,atten_3 = self.cross_attention(atten_1,atten_2,atten_3)
        
        #print("atten_1.shape2: ",atten_1.shape)
...
if __name__ =="__main__":
    ...
    tans = SelfAttention_CrossAttention_Transformer(dim=514, out_dim=256)
