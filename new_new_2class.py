...
def train(train_loader, model, criterion, optimizer, epoch,labels,multi_M):
    ...
    for i, stuff  in enumerate(train_loader):
        if multi_M:
            (img,t1_img,t2_img,_,target, saffix, sradiomics) = stuff
        else:
            (img,_,target, saffix, sradiomics) = stuff
...
def single_batchav_train(train_loader, model, criterion, optimizer, epoch,labels,multi_M):
    ...
    for i, stuff  in enumerate(train_loader):
        ...
        out,feat = model(input_img,input_affix,input_radiomics)
        auged_out,auged_feat = model(input_auged_img,input_affix,input_radiomics)
...
def main(output_path):
    ...
    for fold, (train_idx,val_idx) in enumerate(splits.split(np.arange(len(total_comman_total_file)),y_box)):
        ...
        if opt.model == "ResNet18":
            ...
            best_pretrained_model_path = opt.best_pretrained_model_path
            ...
            best_model_path = best_pretrained_model_path+"ResNet18"+'_'+"CE"+opt.ps+pps+'_best_model.pth.tar'
            #print("pretrained_best_model_path: ",best_model_path)
            try:
                checkpoint = torch.load(best_model_path)
                model.load_state_dict(checkpoint['state_dict'])
            except:
