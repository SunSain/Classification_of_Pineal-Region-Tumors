...
class DIY_Folder_3M(torch.utils.data.Dataset):
    def __init__(self, num_classes,data_path, loader=nii_loader,train_form = "None",root_mask_radiomics="",use_radiomics=True,istest=False,sec_pair_orig=False,multiclass = True,vice_path = None,multi_M= False
                 ,t1_path ="", t2_path="", t1c_path ="",extra_test=False):
    ...
    def process(self):
        for i, f in enumerate(self.sub_fns):
            ...
            original_data = ( sid, slabel, inforbox,paths)
    ...
    def select_dataset(self, data_idx=[], aug=True, aug_form="Random_Mask",use_secondimg=False,noSameRM=False,usethird=True,comman_total_file=[]):
        ...
        for i, idx in enumerate(data_idx):
            (sid, slabel, inforbox,paths) = self.dataset[comman_total_file[idx]]
            a1 =  (sid, slabel, inforbox,paths)
...
if __name__=="__main__":
    ...
    data = DIY_Folder_3M(data_path=data_path, loader=nii_loader,train_form = "None",root_mask_radiomics="/opt/chenxingru/Pineal_region/after_12_08/after_12_08/segmentation/DATASET/data_raw/nnUNet_raw_data/Task009_myT1c/labelsTr/",use_radiomics=False,istest=False,sec_pair_orig=False)
